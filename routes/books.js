...
router.get('/:id', async (req, res) => {
    ...
    try{
        const response = await axios.get(`https://liber-api.onrender.com/book/${bookId}`);
        res.json(response.data);
    }
    catch(err){
        console.error(err);
...
router.get('/images', async (req, res) => {
    try{
        const response = await axios.get("https://liber-api.onrender.com/book/images");
        res.json(response.data);
    }
    catch(err){
        console.error(err);
...
router.post('/images/:id', async (req, res) => {
    ...
    try{
        await axios.post(`https://liber-api.onrender.com/book/images/${bookId}`, req.body);
        res.json("Imagem cadastrada com sucesso!");
    }
    catch(err){
        console.error(err);
        res.status(500).json({ err: 'Erro ao enviar as informações da imagem do livro' })
    }
})
